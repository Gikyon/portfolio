...
namespace sdds {
	...
	struct Profile {
		...
		bool validateAddress(){
			int length;
			length = sizeof(m_address.postal_code) / sizeof(m_address.postal_code[0]);

			if (m_address.number < 0 ){
				throw std::invalid_argument("Numbers cannot be negative!");
				return false;
			}
			else if (length < Address_postal_code_length){
				throw std::invalid_argument("Length of the postal code is invalid!");
				return false;
			} else {return true;}

			std::regex postalcode ("[A-Z][0-9][A-Z]( )[0-9][A-Z][0-9]");
			bool flag;
			// checking postal code with regex //
			if (!std::regex_match(m_address.postal_code, postalcode)){
